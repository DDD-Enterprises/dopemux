...
class TaskMapping:
    ...
    def generate_hash(self, leantime_task: Optional[LeantimeTask] = None,
                     taskmaster_task: Optional[TaskMasterTask] = None) -> str:
        ...
        if leantime_task:
            data['leantime'] = {
                ...
                'updated_at': leantime_task.updated_at.isoformat() if leantime_task.updated_at else None
            }
        if taskmaster_task:
            data['taskmaster'] = {
                ...
                'updated_at': taskmaster_task.updated_at.isoformat() if taskmaster_task.updated_at else None
            }
...
class LeantimeTaskMasterSyncManager:
    ...
    async def initialize(self, leantime_client: LeantimeMCPClient,
                        taskmaster_client: TaskMasterMCPClient) -> bool:
        """
        ...
        try:
            ...
            if not (leantime_health.get('connected') and taskmaster_health.get('connected')):
    ...
    async def sync_all(self) -> SyncResult:
        ...
        try:
            ...
            logger.info(f"Found {len(leantime_tasks)} Leantime tasks, {len(taskmaster_tasks)} TaskMaster tasks")

            # Sync from Leantime to TaskMaster
            lt_to_tm_result = await self._sync_leantime_to_taskmaster(leantime_tasks, taskmaster_tasks)
            ...
            tm_to_lt_result = await self._sync_taskmaster_to_leantime(taskmaster_tasks, leantime_tasks)
            ...
            sync_result.synced_tasks = sync_result.created_tasks + sync_result.updated_tasks
            self.last_sync = start_time

            # Save updated mappings
            await self._save_task_mappings()

            logger.info(f"Sync completed: {sync_result.synced_tasks} tasks synced, {sync_result.conflicts} conflicts")
    ...
    async def _sync_leantime_to_taskmaster(self, leantime_tasks: List[LeantimeTask],
                                          taskmaster_tasks: List[TaskMasterTask]) -> SyncResult:
        ...
        for lt_task in leantime_tasks:
            try:
                ...
                if mapping and mapping.taskmaster_id:
                    ...
                    if tm_task:
                        if await self._should_sync_to_taskmaster(lt_task, tm_task, mapping):
    ...
    async def _sync_taskmaster_to_leantime(self, taskmaster_tasks: List[TaskMasterTask],
                                          leantime_tasks: List[LeantimeTask]) -> SyncResult:
        ...
        for tm_task in taskmaster_tasks:
            try:
                ...
                if mapping and mapping.leantime_id:
                    ...
                    if lt_task:
                        if await self._should_sync_to_leantime(tm_task, lt_task, mapping):
    ...
    async def _should_sync_to_taskmaster(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                                       mapping: TaskMapping) -> bool:
        """Determine if Leantime task should be synced to TaskMaster."""
        # Generate current hash for conflict detection
        current_hash = mapping.generate_hash(leantime_task=lt_task, taskmaster_task=tm_task)
    ...
    async def _should_sync_to_leantime(self, tm_task: TaskMasterTask, lt_task: LeantimeTask,
                                     mapping: TaskMapping) -> bool:
    ...
    def _has_conflict(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                     mapping: TaskMapping) -> bool:
        ...
        return (lt_updated > mapping.last_sync and
                tm_updated > mapping.last_sync and
                abs((lt_updated - tm_updated).total_seconds()) < 300)  # Within 5 minutes

    async def _handle_conflict(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                             mapping: TaskMapping):
        ...
        if self.conflict_resolution == ConflictResolution.LEANTIME_WINS:
            ...
            logger.warning(f"Manual review required for conflict: LT:{lt_task.id} <-> TM:{tm_task.id}")
            # Store conflict for manual resolution
            await self._store_conflict_for_review(lt_task, tm_task, mapping)

    async def _intelligent_merge(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                               mapping: TaskMapping):
        ...
        # Update mapping
        mapping.sync_hash = mapping.generate_hash(leantime_task=lt_task, taskmaster_task=tm_task)
    ...
    async def _create_leantime_task(self, tm_task: TaskMasterTask, mapping_key: str):
        ...
        lt_task = LeantimeTask(
            ...
            status=self._map_status_to_leantime(tm_task.status),
            priority=TaskPriority(str(min(tm_task.priority, 4))),
            story_points=int(tm_task.estimated_hours) if tm_task.estimated_hours else None
        )
    ...
    async def _update_taskmaster_task(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                                    mapping: TaskMapping):
    ...
    async def _update_leantime_task(self, tm_task: TaskMasterTask, lt_task: LeantimeTask,
                                  mapping: TaskMapping):
    ...
    async def _store_conflict_for_review(self, lt_task: LeantimeTask, tm_task: TaskMasterTask,
                                       mapping: TaskMapping):
    ...
    async def _load_task_mappings(self):
        ...
        mappings_file = Path(self.config.get('sync.mappings_file', 'task_mappings.json'))
            ...
            try:
                ...
                for key, mapping_data in data.items():
                    self.task_mappings[key] = TaskMapping(
                        ...
                        last_sync=datetime.fromisoformat(mapping_data.get('last_sync', datetime.now().isoformat())),
                        conflict_count=mapping_data.get('conflict_count', 0),
                        sync_direction=SyncDirection(mapping_data.get('sync_direction', 'bidirectional'))
                    )
    ...
    async def sync_project_from_prd(self, prd_content: str, project_name: str,
                                   leantime_project_id: Optional[int] = None) -> SyncResult:
        """
        ...
        try:
            ...
            prd_analysis = await self.taskmaster_client.parse_prd(prd_content, project_name)
            ...
            if leantime_project_id:
                ...
                project = await self.leantime_client.create_project(
                    LeantimeProject(name=project_name, description=f"Generated from PRD analysis")
                )
            ...
            logger.info(f"PRD sync completed: {result.created_tasks} tasks created from {len(prd_analysis.tasks)} analyzed")
    ...
    async def get_sync_status(self) -> Dict[str, Any]:
        ...
        leantime_health = await self.leantime_client.health_check() if self.leantime_client else {"status": "not_connected"}
        taskmaster_health = await self.taskmaster_client.health_check() if self.taskmaster_client else {"status": "not_connected"}
        ...
        return {
            ...
            "total_conflicts": sum(mapping.conflict_count for mapping in self.task_mappings.values())
        }
