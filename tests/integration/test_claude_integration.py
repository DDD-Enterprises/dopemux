            ...
            with patch('dopemux.config.manager.ConfigManager._init_paths') as mock_init_paths:
                ...
                with patch.dict('os.environ', {
                    ...
                    context = {
                        ...
                        'open_files': [
                            {'path': 'src/main.py', 'cursor_position': {'line': 10, 'column': 5}}
                        ]
                    }

                    claude_config = launcher._generate_claude_config(project_path, context)
                    ...
                    assert claude_config["env"]["DOPEMUX_LAST_GOAL"] == "Implement feature X"
            ...
            with patch('dopemux.config.manager.ConfigManager._init_paths') as mock_init_paths:
                ...
                with patch('shutil.which', side_effect=mock_which_side_effect):
                    with patch.dict('os.environ', {
                        ...
                        expected_servers = ["mas-sequential-thinking", "context7", "claude-context", "morphllm-fast-apply", "exa"]
