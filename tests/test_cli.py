...
from dopemux.cli import cli, show_version
...
class TestCLI:
    ...
    def test_init_command_success(self, mock_context, mock_configurator, mock_exists, mock_config):
    ...
    @patch('dopemux.cli.ClaudeConfigurator')
    @patch('dopemux.cli.ContextManager')
    def test_init_command_force_overwrite(self, mock_context, mock_configurator, mock_exists, mock_config):
    ...
    def test_start_command_success(self, mock_attention, mock_launcher, mock_context, mock_cwd, mock_config):
    ...
    def test_status_command_all_metrics(self, mock_decomposer, mock_context, mock_attention, mock_config):
        ...
        mock_task_decomposer.get_progress.return_value = {
            'tasks': [
                {'name': 'Task 1', 'completed': True, 'in_progress': False, 'progress': 1.0},
                {'name': 'Task 2', 'completed': False, 'in_progress': True, 'progress': 0.5}
            ]
        }
            ...
            with patch('dopemux.cli.Path.exists', return_value=True):
                with patch('dopemux.cli.ClaudeLauncher') as mock_launcher:
                    with patch('dopemux.cli.AttentionMonitor'):
                        result = runner.invoke(cli, ['start', '--session', 'session-123'])
            ...
            with patch('dopemux.cli.Path.exists', return_value=True):
                with patch('dopemux.cli.Path.resolve', return_value=Path('/test')):
                    with patch('dopemux.cli.ClaudeConfigurator') as mock_configurator:
                        with patch('dopemux.cli.ContextManager'):
                            result = runner.invoke(cli, ['init', '--template', 'rust', '.'])
